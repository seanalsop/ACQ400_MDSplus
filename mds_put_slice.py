...
def mds_put_slice(args):
    ...
    if args.tlatch_report:
        cols[:, 1] = do_tlatch_report(
            cols[:, store_cols[0]], args.tlatch_report)
...
def run_main():
    parser = argparse.ArgumentParser(
        description='mds_put_slice slice a data file and submit to MDSplus')
    parser.add_argument('--ncols', type=int, default=64,
                        help="number of columns in data")
    parser.add_argument('--dtype', type=str, default='np.uint32',
                        help="data type np.uint32, np.int16 etc")
    parser.add_argument('--shr', type=int, default=0, help='right shift')
    parser.add_argument('--store_cols', default=':', type=str,
                        help="list of column indices to store")
    parser.add_argument('--node_name', type=str,
                        help="node name %d format accepted")
    parser.add_argument('--default_node', type=str, help="default node")
    parser.add_argument('--tlatch_report', type=int, default=0,
                        help="1: brief tlatch check, 2: detail tlatch check")
